...
(function (global, factory) {
    ...
    class Layout {
        ...
        holdTransition() {
            ...
            window.addEventListener('resize', () => {
                document.getElementById('body').classList.add(CLASS_NAME_HOLD_TRANSITIONS);
                clearTimeout(resizeTimer);
                resizeTimer = setTimeout(() => {
                    document.getElementById('body').classList.remove(CLASS_NAME_HOLD_TRANSITIONS);
                }, 400);
            });
        }
    }
    onDOMContentLoaded(() => {
        const data = new Layout(document.getElementById('body'));
        data.holdTransition();
        setTimeout(() => {
            document.getElementById('body').classList.add(CLASS_NAME_APP_LOADED);
        }, 400);
    });
    ...
    class PushMenu {
        ...
        expand() {
            ...
            document.getElementById('body').classList.remove(CLASS_NAME_SIDEBAR_COLLAPSE);
            document.getElementById('body').classList.add(CLASS_NAME_SIDEBAR_OPEN);
            this._element.dispatchEvent(event);
        }
        collapse() {
            const event = new Event(EVENT_COLLAPSE);
            document.getElementById('body').classList.remove(CLASS_NAME_SIDEBAR_OPEN);
            document.getElementById('body').classList.add(CLASS_NAME_SIDEBAR_COLLAPSE);
            this._element.dispatchEvent(event);
        }
        addSidebarBreakPoint() {
            ...
            if (window.innerWidth <= this._config.sidebarBreakpoint) {
                ...
                if (!document.getElementById('body').classList.contains(CLASS_NAME_SIDEBAR_MINI)) {
                    this.expand();
                }
                if (document.getElementById('body').classList.contains(CLASS_NAME_SIDEBAR_MINI) && document.getElementById('body').classList.contains(CLASS_NAME_SIDEBAR_COLLAPSE)) {
                    this.collapse();
                }
            }
        }
        toggle() {
            if (document.getElementById('body').classList.contains(CLASS_NAME_SIDEBAR_COLLAPSE)) {
                this.expand();
            }
            else {
                this.collapse();
